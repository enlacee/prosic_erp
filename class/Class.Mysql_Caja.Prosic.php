...
class Mysql_Caja_Prosic {

    private $hostname_conexion = "localhost";
    private $database_conexion = "dbprosic_server_caja";
    private $username_conexion = "root";
    private $password_conexion = "dbc4mu2010";
//    
...
//return $sql;
    ...
    function textarea($etiqueta, $nombre, $defecto='', $atributos='') {
        $retorna = '<label>' . $etiqueta . '</label>';
        $retorna.= '<textarea id="' . $nombre . '" name="' . $nombre . '" rows="3" cols="65" ' . $atributos . '>' . $defecto . '</textarea>';
        return $retorna;
    }
     ...
     * @filesource
     */
    function selected($tabla, $id, $etiqueta, $defecto='', $newsql='', $status='', $extra='') {
        ...
        while ($row_select = mysql_fetch_array($res)) {
            ...
            $retorna.= "<option value='" . $row_select[0] . "' $selected>" . utf8_encode($row_select[2]) . "</option>";
        }
    ...
    function cargar_nombre_campo($tabla, $post) {
		...
		WHERE TABLE_SCHEMA = 'dbprosic_server_caja' AND TABLE_NAME = '" . $tabla . "'";
